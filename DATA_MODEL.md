...
| Date | Run | Summary |
|---|---|---|
| 2026-07-30 | `fix/consumer-verify-cli-paths` @ `971c942` | Consumer verify CLI primary form is `node "<CURSOR_PLUGIN_ROOT\|~/.cursor/plugins/local/cursor-harness>/scripts/{record-verify,ci-parity}.mjs"` via `verify-cli-paths.mjs` (`verifyPluginScriptsRoot`, `formatRecordVerifyCommand`, `formatCiParityCommand`); `npm run verify:record` / `verify:ci-parity` harness dogfood only. Updated `VerifyLedger` / `VerifyProfile` / `VerifyRecordCliResult` ingestion; agent/stop messages + `formatRecordRecipe` use the helpers. |
| 2026-07-30 | `feat/consumer-verify-before-push` @ `2080edf` | Tier 1 security: `resolveGitWorkTreeFromArgv` binds `git push` ledger root to `-C`/`--git-dir`/`--work-tree` (fail closed); `normalizeVerifyCmd` shared by profile write + custom coverage; sidecar cmds must pass `!verifyCommandIsTrivial` + `matchesCustomVerification` (rejects inert `go version`); justfile/Makefile extract filtered by `looksLikeCheckCommand`; custom allowlist adds ruff/flake8/mypy/pylint/eslint/prettier/black/isort. |
| 2026-07-30 | `feat/consumer-verify-before-push` | G-data-document merge: re-verified `VerifyProfile` / `VerifyLedger` custom-sidecar coverage against `ci-parity-lib.mjs` + `verify-ledger-lib.mjs`; tightened property notes (`isValidVerifyProfile` string rules, `recommendProfile` heuristics, `impl_verified`/`computeImplVerified` pass `root`); `PluginInventory` components **377** (script 20, command 11). |
| 2026-07-30 | `feat/consumer-verify-before-push` | Added `VerifyProfile` sidecar (`.cursor/verify-profile.json` v1) via `ci-parity` CLI (`npm run verify:ci-parity` / `plugin/scripts/ci-parity.mjs`); `VerifyLedger` `custom` coverage requires exact listed cmds when sidecar present; command `/verify-ci-parity`; gate-dag `impl-verified` consumer step. |
| 2026-07-30 | `feat/consumer-verify-before-push` | `BeforeShellExecutionHook` — plain `git push` (after Tier-A force / `git-remote-ref-delete`) deny with new rule `git-push-without-verify` + `GIT_PUSH_WITHOUT_VERIFY_AGENT_MESSAGE` when ledger invalid for HEAD; `VERIFY_PR_GATE_DISABLED=1` skips both push and PR shell gates. Cataloged `VerifyLedgerStopHook` (`verify-ledger-stop.mjs` / `verify-ledger-stop-lib.mjs`, hooks.json `stop` `loop_limit: 1`) — follow-up when `status=completed`, dirty or ahead of upstream, and ledger invalid. `VerifyLedger` rust clippy coverage requires `--all-targets` and `-D warnings`/`-Dwarnings` (`matchesCargoClippy`). |
| 2026-07-29 | `feat/gh-hardening` | `BeforeShellExecutionHook` — position-aware `gh` verb resolution (`resolveGhVerb`, before-shell-execution.mjs:631-661): all Tier A/B checks AND the `gh repo delete` / `gh release delete` denials now test the resolved verb — the first non-flag word after the group name, consuming values of `-R`/`--repo`/`--hostname` (incl. glued `-Ro/r` and `--repo=o/r` forms; unrecognized pre-verb flags fail closed as ambiguous) — closing flag-before-verb evasion (`gh pr -R o/r merge 12`, `gh auth --hostname x token`, leaf-flag forms `gh pr -b x merge 12`, …). Unresolvable verb position (`--` before the verb) fails closed: Tier-A-shaped groups deny with their most restrictive hard rule via `GH_AMBIGUOUS_VERB_RULES` (before-shell-execution.mjs:690-699, wired 853-864); Tier-B-shaped groups (`secret`/`variable`/`workflow`) ledger-gate (before-shell-execution.mjs:857-863). `gh release upload` and `gh release delete-asset` join the `gh-release-mutation` hard deny (before-shell-execution.mjs:899-906). |
| 2026-07-29 | `feat/gh-hardening` | `BeforeShellExecutionHook` — two-tier `gh` hardening. Tier A hard denies (no ledger exception): `gh ssh-key add` / `gh gpg-key add` (rule `gh-account-key-add`), `gh extension install|upgrade` (`gh-extension-install`), `gh alias set|delete` (`gh-alias-mutation`), all `gh pr merge` forms (`gh-pr-merge`), `gh release create|edit` (`gh-release-mutation`), `gh auth token` (`gh-auth-token`) — before-shell-execution.mjs:798-833. Tier B verify-ledger gates (same `gh-pr-without-verify` rule as `gh pr create`): `gh secret set|delete`, `gh variable set|delete`, `gh workflow run|disable|enable`, `gh pr close|reopen`, `gh pr review --approve|--request-changes`, `gh repo create|fork|rename` — before-shell-execution.mjs:846-872. Read-only `gh` forms (`view`/`list`/`status`/`checks`/`diff`, `pr review --comment`, `auth status`) stay ungated. |
| 2026-07-28 | `feat/audit-tier1-fixes` | Behavior fixes (commit 158735c): `BeforeShellExecutionHook` — `git-remote-ref-delete` now also denies exact tokens `--mirror` and `--prune` on `git push` (mirror force-updates AND deletes remote refs; prune deletes refs whose local counterpart is gone); `VerifyLedger` — `custom` profile build-runner matchers consume value-flag operands via `BUILD_VALUE_TAKING_FLAGS` + `hasMeaningfulBuildTarget` (`make -C src`, `ninja -j 8` no longer count as explicit targets), and re-record supersede now removes ALL cmd-equal entries (not just the first) so duplicate-failure legacy ledgers recover on one passing re-record. |
| 2026-07-28 | `feat/audit-tier1-fixes` | Drift fixes: removed dead `write_tools`/`research_tools` keys from `DispatchGateConfig` (lists hardcoded in `dispatchGateIsWriteTool`/`dispatchGateIsResearchTool`); `VerifyLedger` — per-binary meaningful-subcommand allowlists in `custom` profile coverage (`go` test/vet/build, `cmake --build`, `make`/`ninja` explicit targets), identical-cmd supersede on re-record (latest wins), stale-lock (>30s mtime) self-recovery; `BeforeShellExecutionHook` — new denials for mutating `gh api` (method/body-flag detection, verify-ledger gated via `gh-pr-without-verify`) and `git push` refspec-delete (`git-remote-ref-delete`); `GatePlanResult` dispatch-brief rule requires a non-empty change-id segment (JS aligned to bash); `PluginInventory` 372 components, `reference` kind now covering `plugin/references/*` + `plugin/.cursor/*`. |
| 2026-07-28 | `feat/openspec-first-class` | Cataloged `OpenSpecChange` and `OpenSpecSpecDelta` — the on-disk planning layout under `openspec/` (proposal/design/tasks/dispatch briefs and spec deltas) plus the `openspec` CLI validation/archive route. G-data-document review pass verified every row against sources and CLI v1.6.0; corrected `design.md` section names and the RENAMED delta note. `GatePlanResult` updated: openspec docs-only allowlist, dispatch/specs sensitive classification, fail-closed residue. |
...
| **Source** | `plugin/hooks/hooks.json` (`hooks.beforeShellExecution`); `plugin/scripts/before-shell-execution.mjs` (`decision`, `inspectCommand`, `ghCommand`, `resolveGhVerb`, `ghApiIsMutation`, `readInput`, `main`); `plugin/scripts/lib/verify-cli-paths.mjs` (`formatRecordVerifyCommand` — embedded in deny `agent_message`s); `plugin/scripts/lib/verify-ledger-lib.mjs` (`resolveGitWorkTreeFromArgv`, `verifyLedgerAllowsGhPr`, `verifyLedgerProjectRoot`, `GH_PR_WITHOUT_VERIFY_RULE`, `GH_PR_WITHOUT_VERIFY_AGENT_MESSAGE`, `GIT_PUSH_WITHOUT_VERIFY_RULE`, `GIT_PUSH_WITHOUT_VERIFY_AGENT_MESSAGE`); `tests/security/before-shell-execution.test.mjs` (`runHook`, contract assertions) |
...
| `cwd` | string | no | Base for `verifyLedgerProjectRoot` (PR/Tier-B gates) and for `resolveGitWorkTreeFromArgv(..., cwd)` on plain `git push` (relative `-C`/`--work-tree`/`--git-dir`); falls back to `workspace_roots[0]`, `CURSOR_PROJECT_DIR`, then `process.cwd()` |
...
| `agent_message` | string | deny only | Default: generic “guard denied this command” instruction; for rule `gh-pr-without-verify`, replaced by `GH_PR_WITHOUT_VERIFY_AGENT_MESSAGE`; for rule `git-push-without-verify`, replaced by `GIT_PUSH_WITHOUT_VERIFY_AGENT_MESSAGE` (both embed `formatRecordVerifyCommand` spawn hint as primary consumer form; npm `verify:record` labeled harness dogfood only; ledger/`VERIFY_PR_GATE_DISABLED` guidance covering both push and PR) |

Input must be a JSON object no larger than 1 MiB. Policy is default-deny allowlist composed as: allow named `eval` exceptions → deny active expansions (including ANSI-C `$'...'` quoting, `$()`, backticks, and process substitutions) → peel known wrappers/launchers (including Homebrew GNU `gtimeout` / `gnice` / `gstdbuf` / `gtime`) → deny any `GIT_CONFIG_*` assignment in the segment → structurally re-check remaining argv words whose basename is a high-impact executable (`rm`, `git`, `gh`, `npm`, `pnpm`, `busybox`) → deny high-impact resolved shapes → allow only safe literal command forms → else deny. Every pipeline segment must use a literal path-like command word (no glob, brace, tilde, or other expansion metacharacters in command position), optional leading safe assignments (non-`GIT_CONFIG_*`), optional wrappers (`env`, `sudo`, `command`, `builtin`, `nohup`), and optional command launchers (`timeout`, `nice`, `busybox`, `time`, `stdbuf` and their `g*` GNU forms) whose operands are re-inspected. Absolute paths are reduced to basenames for wrapper, launcher, and interpreter recognition. Shell interpreters (`sh`, `bash`, `zsh`, and related) with `-c` are allowlisted only when the script payload recursively satisfies the same policy (maximum depth three). Active command substitutions (`$()`, backticks), process substitutions (`<(...)`, `>(...)`), and ANSI-C quotes (`$'...'`) are denied anywhere they expand; ordinary single-quoted text is inert. `eval` is denied except the exact named forms `eval "$(direnv hook zsh)"` and `eval "$(ssh-agent -s)"` (trim-insensitive). `.` / `source` require a literal safe script path. High-impact shapes denied even as literals include recursive force `rm` (`-rf` / `-fr` / `--recursive --force` and equivalents on any target), destructive Git forms (`reset --hard`, forced `clean`, force-push, force branch delete, and remote ref deletion — `git push --delete|-d <remote> <ref>`, exact tokens `--mirror` (force-updates AND deletes remote refs) and `--prune` (deletes refs whose local counterpart is gone), or an empty-src `:ref` refspec; exact-token matching only, so legitimate `src:dst` refspecs and plain pushes are unaffected; rule `git-remote-ref-delete`, before-shell-execution.mjs:826-834), remaining plain `git push` forms after those Tier-A force/ref-delete denials unless ledger validates for the **effective git work tree** from `resolveGitWorkTreeFromArgv(gitArgv, workspaceRoot)` — global options `-C` / `--git-dir` / `--work-tree` (incl. `=` forms; stacked `-C` left-to-right; prefer `--work-tree`, else parent of `…/.git` when `--git-dir` ends in `/.git`, else final `-C`/`cwd`; bare/nonstandard git-dir without `--work-tree` or unresolvable path → `null` → fail closed) via `verifyLedgerAllowsGhPr(ledgerRoot)` (rule `git-push-without-verify`, distinct from `gh-pr-without-verify`; before-shell-execution.mjs:839-851; agent_message from `GIT_PUSH_WITHOUT_VERIFY_AGENT_MESSAGE`) — not merely the hook cwd, `git -c` assignments that bind shell-running values (`alias.*`, values containing `!`, `core.pager`, `diff.external`, and related keys), `git --config-env` / `--config-env=*` (fail-closed; value is opaque in the command string), `GIT_CONFIG_*` environment assignments (fail-closed on unknown names in that family), selected `gh` and `npm`/`pnpm` mutation forms (after peeling `gh` global options via `ghCommand`, including `-R`/`--repo`/`--hostname`), `gh pr create` / `gh pr ready` unless `verifyLedgerAllowsGhPr(workspaceRoot)` succeeds for the current HEAD (rule `gh-pr-without-verify`; emergency skip `VERIFY_PR_GATE_DISABLED=1` — skips both the push and PR shell-hook ledger checks, not CI), mutating `gh api` calls routed through the same verify-ledger gate (`ghApiIsMutation`, before-shell-execution.mjs:709-751: an explicit `-X`/`--method` of `POST`/`PUT`/`PATCH`/`DELETE` — including glued `-XPOST` and `--method=` forms — or the implicit POST gh performs when body flags `-f`/`-F`/`--field`/`--raw-field`/`--input` are present; read-only GET forms are not gated; wired at before-shell-execution.mjs:954-959, same `gh-pr-without-verify` rule), and mutations or redirects targeting evaluator/canary paths. Malformed JSON, null, arrays, absent or invalid `command` values, oversized input, malformed shell tokenization, and unterminated substitutions return the deterministic `deny` shape using rule `invalid-hook-input`. Other denials name a policy rule such as `command-expansion`, `unsafe-command-word`, `destructive-filesystem-delete`, `git-config-injection`, `git-config-env-injection`, `git-push-without-verify`, `gh-pr-without-verify`, or `eval-not-allowlisted`. Allowed commands return only `{ "permission": "allow" }`. Pipe-into-interpreter and `find -delete` remain explicit residual risks.

The `gh` high-impact rule applies two hardening tiers after peeling `gh` global options via `ghCommand` and resolving the post-group verb via `resolveGhVerb` (before-shell-execution.mjs:631-685): the verb is the first non-flag word after the group name, consuming values of `-R`/`--repo`/`--hostname` (including glued `-Ro/r` and `--repo=o/r` forms; any unrecognized non-`--flag=value` flag before the verb fails closed as ambiguous — cobra skips an unknown flag AND its value when locating subcommands, so its arity cannot be known), so flags between the group name and the verb cannot evade any check. All Tier A/B checks and the `gh repo delete` / `gh release delete` denials test this resolved verb, not the first post-group word. When the verb position cannot be resolved (`--` before the verb), the guard fails closed (before-shell-execution.mjs:850-863): Tier-A-shaped groups deny with their most restrictive hard rule (`GH_AMBIGUOUS_VERB_RULES`, before-shell-execution.mjs:690-699) and Tier-B-shaped groups (`secret`/`variable`/`workflow`) route through the verify-ledger gate. Tier A hard denies (before-shell-execution.mjs:873-910) return a distinct rule and are never softened by a verify ledger: `gh ssh-key add` / `gh gpg-key add` → `gh-account-key-add` (persistent account takeover), `gh extension install` / `gh extension upgrade` → `gh-extension-install` (supply-chain code execution), `gh alias set` / `gh alias delete` → `gh-alias-mutation` (alias expansions are invisible to hook command matching, bypassing every other gate), every `gh pr merge` form (including `--auto`, `--squash`, `--delete-branch`) → `gh-pr-merge` (repo doctrine: humans merge; an agent merge bypasses the ship-gate DAG), `gh release create` / `gh release edit` / `gh release upload` / `gh release delete-asset` → `gh-release-mutation` (releases — and their assets — are owned by `release.yml` and the tag-from-main CI guard), and `gh auth token` → `gh-auth-token` (prints the OAuth credential to stdout). Tier B verify-ledger gates (before-shell-execution.mjs:922-949) route through `verifyLedgerAllowsGhPr` and return rule `gh-pr-without-verify` when no valid ledger proves `impl_verified` for the current HEAD: `gh secret set|delete`, `gh variable set|delete`, `gh workflow run|disable|enable`, `gh pr close|reopen`, `gh pr review --approve|--request-changes` (the `--comment` form stays allowed), and `gh repo create|fork|rename`. Read-only `gh` forms — `pr view|list|status|checks|diff`, `release view|list`, `workflow list|view`, `secret list`, `repo view|list`, `alias list`, `auth status` — are not gated, including flag-before-verb forms such as `gh pr -R o/r view 12` and flag-after-verb forms such as `gh pr review 12 -R o/r --comment`.
...
|---|---|
| **Kind** | `persistence` |
| **Ingestion route** | Per-workspace JSON at `<project>/.cursor/verify-ledger.json` (gitignored); directory lock via `mkdir` at `.cursor/verify-ledger.json.lock` (max ~1s wait; stale-lock self-recovery — a lock dir whose mtime is older than 30s (`LOCK_STALE_MS`, verify-ledger-lib.mjs:43) is treated as a killed-hook leftover and removed by `breakStaleLockDir` (verify-ledger-lib.mjs:56-73) inside the `verifyLedgerLock` retry loop (verify-ledger-lib.mjs:104-117)); **consumer write:** `node "<root>/scripts/record-verify.mjs" --profile <node-harness\|rust\|custom> --run -- <cmd>` where `<root>` is `verifyPluginScriptsRoot()` (`CURSOR_PLUGIN_ROOT` if non-empty trimmed, else `~/.cursor/plugins/local/cursor-harness`) — string from `formatRecordVerifyCommand` (`verify-cli-paths.mjs`); `verifyLedgerAppendCommand` with `spawned: true` only via `--run`. **Harness dogfood only:** `npm run verify:record` / `node plugin/scripts/record-verify.mjs` in this repo’s package.json. Read by `beforeShellExecution` via `verifyLedgerAllowsGhPr` — for plain `git push`, ledger root is `resolveGitWorkTreeFromArgv` (not hook cwd alone); also before `gh pr create|ready` and mutating `gh api` / Tier-B gh forms; also consulted by the `verify-ledger-stop` stop hook |
| **Source** | `plugin/scripts/lib/verify-cli-paths.mjs` (`verifyPluginScriptsRoot`, `formatRecordVerifyCommand`, `formatCiParityCommand`); `plugin/scripts/lib/verify-ledger-lib.mjs` (`VERIFY_LEDGER_VERSION`, `VERIFY_LEDGER_PROFILES`, `resolveGitWorkTreeFromArgv`, `verifyCommandIsTrivial`, `verifyLedgerProfileCoverage`, `matchesCargoClippy` / `hasDenyWarnings`, `emptyVerifyLedger`, `verifyLedgerAppendCommand`, `verifyLedgerValidateForHead`, `verifyLedgerIsValidForHead`, `verifyLedgerAllowsGhPr`, `VERIFY_PR_GATE_DISABLED_ENV`, `GH_PR_WITHOUT_VERIFY_RULE`, `GH_PR_WITHOUT_VERIFY_AGENT_MESSAGE`, `GIT_PUSH_WITHOUT_VERIFY_RULE`, `GIT_PUSH_WITHOUT_VERIFY_AGENT_MESSAGE` — agent messages embed `formatRecordVerifyCommand("<node-harness\|rust\|custom>", "<cmd>")` then note npm as harness dogfood only); `plugin/scripts/lib/ci-parity-lib.mjs` (`loadVerifyProfile`, `isValidVerifyProfile`, `normalizeVerifyCmd`, `formatRecordRecipe` — re-exports verify-cli-paths helpers); `plugin/scripts/lib/verify-ledger-stop-lib.mjs` (`verifyLedgerHandleStop`, `VERIFY_LEDGER_STOP_FOLLOWUP_MESSAGE`); `plugin/scripts/verify-ledger-stop.mjs`; `plugin/scripts/record-verify.mjs`; `plugin/scripts/ci-parity.mjs`; `scripts/lib/verify-ledger-lib.mjs` (re-export); `package.json` (`verify:record`, `verify:ci-parity` — harness dogfood); `.gitignore` (`/.cursor/verify-ledger.json`, `/.cursor/verify-ledger.json.lock`) |
...
| `impl_verified` | boolean | yes | Set `true` on append when every recorded `exit_code` is `0`, `commands.length >= 1`, and `computeImplVerified` → `verifyLedgerProfileCoverage(profile, commands, root)` holds (optional `root` so `custom` can load `.cursor/verify-profile.json`); else `false` |
...
| `commands` | array<object> | yes | Within a HEAD, appends are keyed by `cmd`: re-recording an **identical cmd string supersedes ALL prior entries with that cmd** (latest wins — `verifyLedgerAppendCommand` filters every cmd-equal entry before pushing the new one, verify-ledger-lib.mjs:936-950), so a flaky failure cannot block the HEAD forever once the same command is re-run and passes, and legacy ledgers holding duplicate failing entries recover on one passing re-record; reset (new empty ledger) when file missing, `version` mismatch, or `head_sha` changes |
...
| `rust` | (1) `cargo fmt` with `--check` (2) `cargo clippy` with `--all-targets` **and** deny-warnings (`matchesCargoClippy` / `hasDenyWarnings`: token `-Dwarnings` or adjacent `-D` + `warnings`, including after `--`) (3) `cargo test` OR `cargo nextest` |
| `custom` | **With sidecar:** when `root` is a non-empty string and `loadVerifyProfile(root)` passes `isValidVerifyProfile` (`version === 1`, non-empty `commands` of non-empty trimmed strings — from `ci-parity-lib.mjs`), each listed command is compared via `normalizeVerifyCmd` on both sides (whitespace collapse + quote unwrap — so profile `pytest -k "not slow"` matches ledger `pytest -k not slow`). Listed cmds with empty norm, `verifyCommandIsTrivial(requiredNorm)`, or failing `matchesCustomVerification` on the normalized argv **never** satisfy coverage (rejects inert sidecar entries such as `go version`). Otherwise each required norm must match a spawned ledger entry with `exit_code === 0`, `!verifyCommandIsTrivial(entry.cmd)`, and `matchesCustomVerification` on the entry argv (1 listed cmd ⇒ require that 1; ≥2 ⇒ require each — no fake second). Invalid/missing sidecar or omitted `root` ⇒ fallback. **Without sidecar:** ≥2 spawned non-trivial **verification-shaped** commands (positive allowlist `matchesCustomVerification`, verify-ledger-lib.mjs: npm/pnpm/yarn/bun scripts, node `--test`/scripts, cargo runners, just/task/tox/nox/pytest/ruff/flake8/mypy/pylint/eslint/prettier/black/isort/mvn/gradle/bazel/sbt/deno/meson/…, python `-m pytest|unittest|mypy|ruff|flake8|pylint`, `scripts/`/`bin/` paths). Build-runner binaries use **per-binary meaningful-subcommand allowlists** — `go` only `test`/`vet`/`build` (`VERIFY_CUSTOM_GO_SUBCOMMANDS`), `cmake` only `--build`, `make` requires an explicit non-help target (`matchesCustomMake`), `ninja` requires an explicit target and rejects `-t` tool forms (`matchesCustomNinja`). `make`/`ninja` target detection runs through `hasMeaningfulBuildTarget` with a per-binary `BUILD_VALUE_TAKING_FLAGS` table (make `-C -f -j -o -W -I`, ninja `-C -f -j -k -l`) that consumes the flag's NEXT argv word as its value (glued forms like `-j4`/`-Csrc` are skipped with nothing to consume), so flag-only/bare invocations such as `make -C src`, `make -f mk`, `ninja -C build`, `ninja -j 8` no longer satisfy coverage; bare/version/env/help forms of those binaries are inert. Identity probes (`pwd`/`date`/`whoami`/`git status`/`node --version`) do not count. Agent deny messages (`GH_PR_WITHOUT_VERIFY_AGENT_MESSAGE` / `GIT_PUSH_WITHOUT_VERIFY_AGENT_MESSAGE`) and stop follow-up note: if `.cursor/verify-profile.json` exists, those listed cmds (after `normalizeVerifyCmd`) must be recorded; stop also mentions `/verify-ci-parity` |

Valid for PR / push (`verifyLedgerValidateForHead` / `verifyLedgerIsValidForHead` / `verifyLedgerAllowsGhPr`) when `version === 2`, profile known, `impl_verified === true`, `head_sha` equals current HEAD, `commands.length >= 1`, every `exit_code === 0`, every `spawned === true`, and profile coverage holds (`verifyLedgerValidateForHead` / `computeImplVerified` pass `root` so custom can load the sidecar). `VERIFY_PR_GATE_DISABLED=1` skips the shell-hook push and PR ledger checks and the `verify-ledger-stop` follow-up (not CI `impl-verified` checkbox). **Residual:** Write-tool forging a full v2 ledger with `spawned: true` remains possible.
...
|---|---|
| **Kind** | `persistence` |
| **Ingestion route** | Per-workspace JSON at `<project>/.cursor/verify-profile.json` (path `VERIFY_PROFILE_RELATIVE_PATH`; **not** gitignored — consumers may commit; `.gitignore` only ignores `verify-ledger.json` / lock); **consumer write:** `node "<root>/scripts/ci-parity.mjs" --write` (`formatCiParityCommand(true)` — same `<root>` as `verifyPluginScriptsRoot()`) / `plugin/scripts/ci-parity.mjs --write [--root <path>]` (`writeVerifyProfile`); **harness dogfood only:** `npm run verify:ci-parity -- --write`. Read by `verifyLedgerProfileCoverage` when `profile === "custom"` via `loadVerifyProfile` |
| **Source** | `plugin/scripts/lib/verify-cli-paths.mjs` (`verifyPluginScriptsRoot`, `formatRecordVerifyCommand`, `formatCiParityCommand`); `plugin/scripts/lib/ci-parity-lib.mjs` (`VERIFY_PROFILE_VERSION`, `VERIFY_PROFILE_RELATIVE_PATH`, `discoverCiSources`, `extractCheckCommands`, `normalizeVerifyCmd`, `recommendProfile`, `formatRecordRecipe` → maps cmds through `formatRecordVerifyCommand`, `writeVerifyProfile`, `loadVerifyProfile`, `isValidVerifyProfile`, `scanCiParity`; re-exports verify-cli-paths helpers); `plugin/scripts/ci-parity.mjs`; `package.json` (`verify:ci-parity` — harness dogfood); `plugin/commands/verify-ci-parity.md`; `plugin/references/gate-dag.md` (consumer `impl-verified` step); `tests/security/ci-parity.test.mjs` |
...
| Name | Type | Required | Notes |
|---|---|---|---|
| `version` | integer | yes | Must be `1` (`VERIFY_PROFILE_VERSION`) for `isValidVerifyProfile` / sidecar coverage to apply |
| `commands` | array<string> | yes | Non-empty; every element a non-empty trimmed string (`isValidVerifyProfile`); ledger coverage compares via `normalizeVerifyCmd` (not raw string equality); `writeVerifyProfile` stores each cmd after `normalizeVerifyCmd` (whitespace collapse + quote unwrap) |
| `source` | string | no | Not checked by `isValidVerifyProfile`; CLI/`writeVerifyProfile` default `"ci-parity"` (empty/non-string coerced to `"ci-parity"`) |

CLI (`ci-parity.mjs`): optional `--write`, `--root <path>` / `--root=…`. Default prints `# profile: <rust|node-harness|custom>` then one `node "<plugin-root>/scripts/record-verify.mjs" --profile … --run -- <cmd>` line per extracted check (`formatRecordRecipe` → `formatRecordVerifyCommand`; header is CLI-only). Exit `0` success, `1` no check commands, `2` usage error. `scanCiParity(root)` → `{ sources, commands, profile, recipe }` where `sources` come from `discoverCiSources` (`.github/workflows/*.{yml,yaml}`, `justfile`/`Justfile`, `Makefile` only — no third-party YAML parser). `extractCheckCommands`: workflow `run:` bodies, justfile recipe bodies, and Makefile target bodies are all filtered with `looksLikeCheckCommand` (cargo/npm/just/make/pnpm/yarn/bun verification-shaped starters; noise denylist for deploy/publish/upload/…) — inert lines such as `echo preparing` are not locked into the sidecar. `recommendProfile`: `rust` only when extracted cmds cover `cargo fmt --check` + `clippy --all-targets` + deny-warnings + `test`/`nextest`; `node-harness` when validate + test; else `custom`.

### VerifyLedgerStopHook

| Field | Value |
|---|---|
| **Kind** | `api` |
| **Ingestion route** | Cursor schema-v1 `stop`, registered in `plugin/hooks/hooks.json` as `node "${CURSOR_PLUGIN_ROOT}/scripts/verify-ledger-stop.mjs"` with `timeout: 5` and `loop_limit: 1` (fail-open — no `failClosed`); reads one JSON event on stdin (max 1 MiB), writes one JSON line on stdout, always exits 0 |
| **Source** | `plugin/hooks/hooks.json` (`hooks.stop` entry for `verify-ledger-stop.mjs`); `plugin/scripts/verify-ledger-stop.mjs` (`main`, `readInput`); `plugin/scripts/lib/verify-cli-paths.mjs` (`formatRecordVerifyCommand` — embedded in follow-up); `plugin/scripts/lib/verify-ledger-stop-lib.mjs` (`verifyLedgerHandleStop`, `verifyLedgerStopOk`, `verifyLedgerStopFollowup`, `gitWorktreeIsDirty`, `gitIsAheadOfUpstream`, `gitNeedsVerifyBeforePush`, `VERIFY_LEDGER_STOP_FOLLOWUP_MESSAGE`); `plugin/scripts/lib/verify-ledger-lib.mjs` (`verifyLedgerIsValidForHead`, `verifyLedgerProjectRoot`, `verifyPrGateDisabled`, `VERIFY_PR_GATE_DISABLED_ENV`); `tests/security/verify-ledger-stop.test.mjs` |
...
```json
{
  "input": {
    ...
    "...": "additional Cursor stop event fields are accepted"
  },
  "output": {}
  | {
...
| Name | Type | Required | Notes |
|---|---|---|---|
| `status` | string | no | Follow-up only when exactly `completed`; any other value (or non-object / null / array payload) yields `{}` |
| `cwd` | string | no | Project root via `verifyLedgerProjectRoot`; falls back to `workspace_roots[0]`, `CURSOR_PROJECT_DIR`, then `process.cwd()` |
| `workspace_roots` | array<string> | no | First element used when `cwd` absent |
| additional fields | unknown | no | Accepted; ignored except for root resolution |

Output:

| Name | Type | Required | Notes |
|---|---|---|---|
| (empty object) | object | yes when no follow-up | `verifyLedgerStopOk()` — no `followup_message` key |
| `followup_message` | string | follow-up only | Exactly `VERIFY_LEDGER_STOP_FOLLOWUP_MESSAGE`: dirty/ahead without valid ledger; primary spawn hint is `formatRecordVerifyCommand("<node-harness\|rust\|custom>", "<cmd>")` (plugin script under `verifyPluginScriptsRoot()`); npm `verify:record` labeled harness dogfood only; notes `/verify-ci-parity` for consumer CI floors; documents `VERIFY_PR_GATE_DISABLED=1` skips push, PR, and this stop follow-up |

Decision path (`verifyLedgerHandleStop`): return `{}` unless `status === "completed"` AND `verifyPrGateDisabled()` is false AND `gitNeedsVerifyBeforePush(root)` (dirty porcelain **or** ahead of upstream via `rev-list --count @{u}..HEAD` > 0, else `status -sb` matching `/\[ahead\b/`) AND `verifyLedgerIsValidForHead(root).ok` is false — then `{ followup_message }`. Missing upstream without an ahead marker is not treated as ahead. Malformed JSON, oversized stdin, or thrown errors: emit `{}` (fail-open).
...
|---|---|
| **Kind** | `api` |
| **Ingestion route** | **Consumer:** `node "<root>/scripts/record-verify.mjs" --profile <node-harness\|rust\|custom> --run -- <command...>` (`formatRecordVerifyCommand` / `verifyPluginScriptsRoot()` — `CURSOR_PLUGIN_ROOT` or `~/.cursor/plugins/local/cursor-harness`). Optional `--conversation-id`; `--profile` required on first write for a HEAD, else omit or must match. Prints one JSON object to standard output after appending to `VerifyLedger`; process exit code propagates the child command’s status. `--cmd`/`--exit` are rejected. **Harness dogfood only:** `npm run verify:record -- …` / `node plugin/scripts/record-verify.mjs` |
| **Source** | `plugin/scripts/record-verify.mjs` (`main` stdout payload); `plugin/scripts/lib/verify-cli-paths.mjs` (`formatRecordVerifyCommand`, `verifyPluginScriptsRoot`); `plugin/scripts/lib/verify-ledger-lib.mjs` (`verifyLedgerAppendCommand`, `verifyLedgerIsValidForHead`, `verifyCommandIsTrivial`); `package.json` (`verify:record` — harness dogfood) |
...
| **Source** | `plugin/.cursor/dispatch-gate.json` (shipped default); `plugin/scripts/lib/dispatch-gate-lib.mjs` (`dispatchGateResolveConfig`, `dispatchGateIsEnabled`, `dispatchGateLoadJsonFile`, `dispatchGateIsWriteTool`, `dispatchGateIsResearchTool`); `scripts/lib/dispatch-gate-lib.mjs` (re-export) |
...
Shipped plugin default sets `enabled: false`. Consumer file at the project root wins entirely when present (not field-merged). Write/research tool classification is **not configurable**: the `write_tools` / `research_tools` keys were removed from the shipped default as dead config — the lists are hardcoded in `dispatchGateIsWriteTool` (lowercase `write` / `strreplace` / `delete` / `applypatch` / `editnotebook` / `search_replace` / `edit_file` / `patch`, dispatch-gate-lib.mjs:299-310) and `dispatchGateIsResearchTool` (`read` / `grep` / `glob` / `semanticsearch` / `sematicsearch`, dispatch-gate-lib.mjs:312-315).
...
| `is_sensitive` | boolean | yes | `true` when paths match the sensitive allowlist (hooks, validate/install/release scripts, gate scripts, `plugin/rules/*`, `plugin/commands/*`, `plugin/references/*`, `SECURITY.md`, `.github/workflows/*`, `openspec/changes/<change-id>/dispatch/*` — requires a non-empty change-id segment, `openspec/specs/*`) |
...
Docs-only path allowlist (skipped unless also library/sensitive): `*.md`, `*.mdc`, `LICENSE`, `NOTICE`, `docs/*`, `.claude/memory/*`, `.claude/ledger/*`, plus OpenSpec planning docs — `openspec/*.md` / `openspec/*.mdc` (nested paths included; bash `*` matches `/`) and `*/.openspec.yaml`. OpenSpec paths under `openspec/changes/*/dispatch/*` and `openspec/specs/*` are sensitive (flagged, not skipped) — the dispatch-brief rule requires a non-empty change-id segment between `changes/` and `/dispatch/` (bash `*` spans `/` but demands the literal segments, so a bare `openspec/changes/dispatch/foo.md` is NOT a brief; the JS twin keeps parity with `/^openspec\/changes\/.+\/dispatch\//` — gate-plan-lib.sh:49-53, dispatch-gate-plan-lib.mjs:60-64); any other file type under `openspec/` fails closed to `is_code_change`. `DATA_MODEL.md` is never docs-only. Alternate formats: text `key=value` lines, `--checkboxes` (one label per line), `--skip-docs-only` (`true`\|`false` only). Checkbox matching is case-insensitive on the agent label with optional surrounding `**` after `- [x]` / `- [X]`. For non-docs-only PRs, `check-pr-ship-gates.sh` always requires an extra `- [x] impl-verified` line (template: local `.cursor/verify-ledger.json` recorded for branch HEAD via `npm run verify:record`); this label is outside `GatePlanResult.checkboxes`.
...
| `components` | array<object> | yes | At least one file-backed component with identity, kind, relative path, and digest; current inventory holds **377** components (by kind: `reference` 273, `skill` 46, `agent` 17, `script` 20, `command` 11, `rule` 9, `hook` 1) — includes `verify-ci-parity` command + `ci-parity` / `ci-parity-lib` scripts |
| `platformCapabilities` | array<object> | yes | At least one explicit capability probe result with evidence |

All objects reject undeclared properties. Plugin/component IDs are lowercase alphanumeric/hyphen strings. Relative paths must be non-empty, non-absolute, and contain no `..` segment. The `reference` kind covers skill support files (`assets/`, `references/`, `readme.md`, `experts/`, `scoring-rubrics/`) plus — since the audit-tier1-fixes pass — `plugin/references/*` and `plugin/.cursor/*` (e.g. `dispatch-gate.json`), which ship inside the plugin tree but were previously undiscovered, leaving hash drift untracked; they are modeled as inert `reference` components (repository-validator.mjs:421-436).
...
├── design.md                   # optional — ## Context / ## Goals / Non-Goals / ## Decisions / ## Risks / Trade-offs
...
| `dispatch/<task-id>.md` | markdown | per task | Harness extension ignored by the CLI parser; carries goal, files_read, files_write, dependencies, conflicts, acceptance, verification, and optional `subagent_type` (`engineer` or a stack specialist; defaults to `engineer`); a tasks.md item without its brief is not dispatchable |
...
| scenario | `#### Scenario: <name>` + WHEN/THEN bullets | ≥1 per requirement | Exactly four hashtags; a scenario at the wrong heading level is dropped from parsing (CLI INFO), then validation fails with an explicit missing-scenario ERROR |
...
| RENAMED mapping | FROM:/TO: | RENAMED only | FROM:/TO: format only |
