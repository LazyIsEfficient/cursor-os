...
| **Source** | `plugin/hooks/hooks.json` (`hooks.beforeShellExecution`); `plugin/scripts/before-shell-execution.mjs` (`decision`, `inspectCommand`, `ghCommand`, `resolveGhVerb`, `ghApiIsMutation`, `readInput`, `main`); `plugin/scripts/lib/verify-ledger-lib.mjs` (`verifyLedgerAllowsGhPr`, `verifyLedgerProjectRoot`, `GH_PR_WITHOUT_VERIFY_RULE`, `GH_PR_WITHOUT_VERIFY_AGENT_MESSAGE`, `GIT_PUSH_WITHOUT_VERIFY_RULE`, `GIT_PUSH_WITHOUT_VERIFY_AGENT_MESSAGE`); `tests/security/before-shell-execution.test.mjs` (`runHook`, contract assertions) |
...
| `cwd` | string | no | Workspace root for verify-ledger push/PR gate; falls back to `workspace_roots[0]`, `CURSOR_PROJECT_DIR`, then `process.cwd()` |
...
|---|---|
| **Kind** | `persistence` |
| **Ingestion route** | Per-workspace JSON at `<project>/.cursor/verify-ledger.json` (gitignored); directory lock via `mkdir` at `.cursor/verify-ledger.json.lock` (max ~1s wait; stale-lock self-recovery — a lock dir whose mtime is older than 30s (`LOCK_STALE_MS`, verify-ledger-lib.mjs:38) is treated as a killed-hook leftover and removed by `breakStaleLockDir` (verify-ledger-lib.mjs:51-68) inside the `verifyLedgerLock` retry loop (verify-ledger-lib.mjs:99-112)); written by `npm run verify:record` / `plugin/scripts/record-verify.mjs` (`verifyLedgerAppendCommand` with `spawned: true` only via `--run`); read by `beforeShellExecution` via `verifyLedgerAllowsGhPr` before allowing plain `git push`, `gh pr create|ready`, and mutating `gh api` / Tier-B gh forms; also consulted by the `verify-ledger-stop` stop hook |
...
| `custom` | **With sidecar:** when `root` is a non-empty string and `loadVerifyProfile(root)` passes `isValidVerifyProfile` (`version === 1`, non-empty `commands` of non-empty trimmed strings — from `ci-parity-lib.mjs`), each listed command must appear as an **exact** `cmd` on a spawned ledger entry with `exit_code === 0` (1 listed cmd ⇒ require that 1; ≥2 ⇒ require each — no fake second). Invalid/missing sidecar or omitted `root` ⇒ fallback. **Without sidecar:** ≥2 spawned non-trivial **verification-shaped** commands (positive allowlist `matchesCustomVerification`, verify-ledger-lib.mjs: npm/pnpm/yarn/bun scripts, node `--test`/scripts, cargo runners, just/task/tox/pytest/mvn/gradle/bazel/…, python `-m pytest|unittest|mypy|ruff|flake8|pylint`, `scripts/`/`bin/` paths). Build-runner binaries use **per-binary meaningful-subcommand allowlists** — `go` only `test`/`vet`/`build` (`VERIFY_CUSTOM_GO_SUBCOMMANDS`), `cmake` only `--build`, `make` requires an explicit non-help target (`matchesCustomMake`), `ninja` requires an explicit target and rejects `-t` tool forms (`matchesCustomNinja`). `make`/`ninja` target detection runs through `hasMeaningfulBuildTarget` with a per-binary `BUILD_VALUE_TAKING_FLAGS` table (make `-C -f -j -o -W -I`, ninja `-C -f -j -k -l`) that consumes the flag's NEXT argv word as its value (glued forms like `-j4`/`-Csrc` are skipped with nothing to consume), so flag-only/bare invocations such as `make -C src`, `make -f mk`, `ninja -C build`, `ninja -j 8` no longer satisfy coverage; bare/version/env/help forms of those binaries are inert. Identity probes (`pwd`/`date`/`whoami`/`git status`/`node --version`) do not count. Agent deny messages (`GH_PR_WITHOUT_VERIFY_AGENT_MESSAGE` / `GIT_PUSH_WITHOUT_VERIFY_AGENT_MESSAGE`) and stop follow-up note: if `.cursor/verify-profile.json` exists, those exact cmds must be recorded; stop also mentions `/verify-ci-parity` |
...
| `commands` | array<string> | yes | Non-empty; every element a non-empty trimmed string (`isValidVerifyProfile`); exact `cmd` match in ledger coverage; `writeVerifyProfile` collapses whitespace via `normalizeCommand` before write |
