...
| **Source** | `plugin/hooks/hooks.json` (`hooks.beforeShellExecution`); `plugin/scripts/before-shell-execution.mjs` (`decision`, `inspectCommand`, `ghCommand`, `ghApiIsMutation`, `readInput`, `main`); `plugin/scripts/lib/verify-ledger-lib.mjs` (`verifyLedgerAllowsGhPr`, `verifyLedgerProjectRoot`, `GH_PR_WITHOUT_VERIFY_RULE`, `GH_PR_WITHOUT_VERIFY_AGENT_MESSAGE`); `tests/security/before-shell-execution.test.mjs` (`runHook`, contract assertions) |
