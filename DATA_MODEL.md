...
| `commands` | array<object> | yes | Within a HEAD, appends are keyed by `cmd`: re-recording an **identical cmd string supersedes the prior entry** (latest wins — `verifyLedgerAppendCommand`, verify-ledger-lib.mjs:899-914), so a flaky failure cannot block the HEAD forever once the same command is re-run and passes; reset (new empty ledger) when file missing, `version` mismatch, or `head_sha` changes |
...
| `custom` | ≥2 spawned non-trivial **verification-shaped** commands (positive allowlist `matchesCustomVerification`, verify-ledger-lib.mjs:548-646: npm/pnpm/yarn/bun scripts, node `--test`/scripts, cargo runners, just/task/tox/pytest/mvn/gradle/bazel/…, python `-m pytest|unittest|mypy|ruff|flake8|pylint`, `scripts/`/`bin/` paths). Build-runner binaries use **per-binary meaningful-subcommand allowlists** — `go` only `test`/`vet`/`build` (`VERIFY_CUSTOM_GO_SUBCOMMANDS`, verify-ledger-lib.mjs:507, 520-522), `cmake` only `--build` (verify-ledger-lib.mjs:524-526), `make` requires an explicit non-help target (verify-ledger-lib.mjs:528-531), `ninja` requires an explicit target and rejects `-t` tool forms (verify-ledger-lib.mjs:533-540); bare/version/env/help forms of those binaries are inert. Identity probes (`pwd`/`date`/`whoami`/`git status`/`node --version`) do not count |
