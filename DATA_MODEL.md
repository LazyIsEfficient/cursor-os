...
| `custom` | ≥2 spawned non-trivial **verification-shaped** commands (positive allowlist `matchesCustomVerification`, verify-ledger-lib.mjs:586-683: npm/pnpm/yarn/bun scripts, node `--test`/scripts, cargo runners, just/task/tox/pytest/mvn/gradle/bazel/…, python `-m pytest|unittest|mypy|ruff|flake8|pylint`, `scripts/`/`bin/` paths). Build-runner binaries use **per-binary meaningful-subcommand allowlists** — `go` only `test`/`vet`/`build` (`VERIFY_CUSTOM_GO_SUBCOMMANDS`, verify-ledger-lib.mjs:506, 557-559), `cmake` only `--build` (verify-ledger-lib.mjs:561-563), `make` requires an explicit non-help target (`matchesCustomMake`, verify-ledger-lib.mjs:565-569), `ninja` requires an explicit target and rejects `-t` tool forms (`matchesCustomNinja`, verify-ledger-lib.mjs:571-578). `make`/`ninja` target detection runs through `hasMeaningfulBuildTarget` (verify-ledger-lib.mjs:532-554) with a per-binary `BUILD_VALUE_TAKING_FLAGS` table (verify-ledger-lib.mjs:526-529 — make `-C -f -j -o -W -I`, ninja `-C -f -j -k -l`) that consumes the flag's NEXT argv word as its value (glued forms like `-j4`/`-Csrc` are skipped with nothing to consume), so flag-only/bare invocations such as `make -C src`, `make -f mk`, `ninja -C build`, `ninja -j 8` no longer satisfy coverage; bare/version/env/help forms of those binaries are inert. Identity probes (`pwd`/`date`/`whoami`/`git status`/`node --version`) do not count |

Valid for PR / push (`verifyLedgerValidateForHead` / `verifyLedgerIsValidForHead` / `verifyLedgerAllowsGhPr`) when `version === 2`, profile known, `impl_verified === true`, `head_sha` equals current HEAD, `commands.length >= 1`, every `exit_code === 0`, every `spawned === true`, and profile coverage holds. `VERIFY_PR_GATE_DISABLED=1` skips the shell-hook push and PR ledger checks and the `verify-ledger-stop` follow-up (not CI `impl-verified` checkbox). **Residual:** Write-tool forging a full v2 ledger with `spawned: true` remains possible.
...
| `components` | array<object> | yes | At least one file-backed component with identity, kind, relative path, and digest; current inventory holds **372** components (by kind: `reference` 273, `skill` 46, `agent` 17, `script` 16, `command` 10, `rule` 9, `hook` 1) |
