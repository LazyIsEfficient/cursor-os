...
| Date | Run | Summary |
|---|---|---|
| 2026-07-29 | `feat/gh-hardening` | `BeforeShellExecutionHook` — position-aware `gh` verb resolution (`resolveGhVerb`, before-shell-execution.mjs:631-661): all Tier A/B checks AND the `gh repo delete` / `gh release delete` denials now test the resolved verb — the first non-flag word after the group name, consuming values of `-R`/`--repo`/`--hostname` (incl. glued `-Ro/r` and `--repo=o/r` forms; unknown non-`--flag=value` flags conservatively consume no value) — closing flag-before-verb evasion (`gh pr -R o/r merge 12`, `gh auth --hostname x token`, …). Unresolvable verb position (`--` before the verb) fails closed: Tier-A-shaped groups deny with their most restrictive hard rule via `GH_AMBIGUOUS_VERB_RULES` (before-shell-execution.mjs:690-699, wired 853-864); Tier-B-shaped groups (`secret`/`variable`/`workflow`) ledger-gate (before-shell-execution.mjs:857-863). `gh release upload` and `gh release delete-asset` join the `gh-release-mutation` hard deny (before-shell-execution.mjs:899-906). |
