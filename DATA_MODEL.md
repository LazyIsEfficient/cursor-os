...
| **Source** | `plugin/hooks/hooks.json` (`hooks.beforeShellExecution`); `plugin/scripts/before-shell-execution.mjs` (`decision`, `inspectCommand`, `ghCommand`, `resolveGhVerb`, `ghApiIsMutation`, `readInput`, `main`); `plugin/scripts/lib/verify-ledger-lib.mjs` (`resolveGitWorkTreeFromArgv`, `verifyLedgerAllowsGhPr`, `verifyLedgerProjectRoot`, `GH_PR_WITHOUT_VERIFY_RULE`, `GH_PR_WITHOUT_VERIFY_AGENT_MESSAGE`, `GIT_PUSH_WITHOUT_VERIFY_RULE`, `GIT_PUSH_WITHOUT_VERIFY_AGENT_MESSAGE`); `tests/security/before-shell-execution.test.mjs` (`runHook`, contract assertions) |
...
|---|---|
| **Kind** | `persistence` |
| **Ingestion route** | Per-workspace JSON at `<project>/.cursor/verify-ledger.json` (gitignored); directory lock via `mkdir` at `.cursor/verify-ledger.json.lock` (max ~1s wait; stale-lock self-recovery — a lock dir whose mtime is older than 30s (`LOCK_STALE_MS`, verify-ledger-lib.mjs:43) is treated as a killed-hook leftover and removed by `breakStaleLockDir` (verify-ledger-lib.mjs:56-73) inside the `verifyLedgerLock` retry loop (verify-ledger-lib.mjs:104-117)); written by `npm run verify:record` / `plugin/scripts/record-verify.mjs` (`verifyLedgerAppendCommand` with `spawned: true` only via `--run`); read by `beforeShellExecution` via `verifyLedgerAllowsGhPr` — for plain `git push`, ledger root is `resolveGitWorkTreeFromArgv` (not hook cwd alone); also before `gh pr create|ready` and mutating `gh api` / Tier-B gh forms; also consulted by the `verify-ledger-stop` stop hook |
| **Source** | `plugin/scripts/lib/verify-ledger-lib.mjs` (`VERIFY_LEDGER_VERSION`, `VERIFY_LEDGER_PROFILES`, `resolveGitWorkTreeFromArgv`, `verifyCommandIsTrivial`, `verifyLedgerProfileCoverage`, `matchesCargoClippy` / `hasDenyWarnings`, `emptyVerifyLedger`, `verifyLedgerAppendCommand`, `verifyLedgerValidateForHead`, `verifyLedgerIsValidForHead`, `verifyLedgerAllowsGhPr`, `VERIFY_PR_GATE_DISABLED_ENV`, `GH_PR_WITHOUT_VERIFY_RULE`, `GH_PR_WITHOUT_VERIFY_AGENT_MESSAGE`, `GIT_PUSH_WITHOUT_VERIFY_RULE`, `GIT_PUSH_WITHOUT_VERIFY_AGENT_MESSAGE`); `plugin/scripts/lib/ci-parity-lib.mjs` (`loadVerifyProfile`, `isValidVerifyProfile`, `normalizeVerifyCmd` — custom sidecar); `plugin/scripts/lib/verify-ledger-stop-lib.mjs` (`verifyLedgerHandleStop`, `VERIFY_LEDGER_STOP_FOLLOWUP_MESSAGE`); `plugin/scripts/verify-ledger-stop.mjs`; `plugin/scripts/record-verify.mjs`; `plugin/scripts/ci-parity.mjs`; `scripts/lib/verify-ledger-lib.mjs` (re-export); `package.json` (`verify:record`, `verify:ci-parity`); `.gitignore` (`/.cursor/verify-ledger.json`, `/.cursor/verify-ledger.json.lock`) |
...
|---|---|
| **Kind** | `persistence` |
| **Ingestion route** | Per-workspace JSON at `<project>/.cursor/verify-profile.json` (path `VERIFY_PROFILE_RELATIVE_PATH`; **not** gitignored — consumers may commit; `.gitignore` only ignores `verify-ledger.json` / lock); written by `npm run verify:ci-parity -- --write` / `plugin/scripts/ci-parity.mjs --write [--root <path>]` (`writeVerifyProfile`); read by `verifyLedgerProfileCoverage` when `profile === "custom"` via `loadVerifyProfile` |
| **Source** | `plugin/scripts/lib/ci-parity-lib.mjs` (`VERIFY_PROFILE_VERSION`, `VERIFY_PROFILE_RELATIVE_PATH`, `discoverCiSources`, `extractCheckCommands`, `normalizeVerifyCmd`, `recommendProfile`, `formatRecordRecipe`, `writeVerifyProfile`, `loadVerifyProfile`, `isValidVerifyProfile`, `scanCiParity`); `plugin/scripts/ci-parity.mjs`; `package.json` (`verify:ci-parity`); `plugin/commands/verify-ci-parity.md`; `plugin/references/gate-dag.md` (consumer `impl-verified` step); `tests/security/ci-parity.test.mjs` |
...
| **Source** | `plugin/hooks/hooks.json` (`hooks.stop` entry for `verify-ledger-stop.mjs`); `plugin/scripts/verify-ledger-stop.mjs` (`main`, `readInput`); `plugin/scripts/lib/verify-ledger-stop-lib.mjs` (`verifyLedgerHandleStop`, `verifyLedgerStopOk`, `verifyLedgerStopFollowup`, `gitWorktreeIsDirty`, `gitIsAheadOfUpstream`, `gitNeedsVerifyBeforePush`, `VERIFY_LEDGER_STOP_FOLLOWUP_MESSAGE`); `plugin/scripts/lib/verify-ledger-lib.mjs` (`verifyLedgerIsValidForHead`, `verifyLedgerProjectRoot`, `verifyPrGateDisabled`, `VERIFY_PR_GATE_DISABLED_ENV`); `tests/security/verify-ledger-stop.test.mjs` |
...
| `followup_message` | string | follow-up only | Exactly `VERIFY_LEDGER_STOP_FOLLOWUP_MESSAGE`: dirty/ahead without valid ledger; instructs `npm run verify:record -- --profile <node-harness\|rust\|custom> --run -- <cmd>`; notes `/verify-ci-parity` for consumer CI floors; documents `VERIFY_PR_GATE_DISABLED=1` skips push, PR, and this stop follow-up |
