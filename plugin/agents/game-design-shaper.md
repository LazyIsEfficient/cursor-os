...
description: Game design pipeline — intake game idea, design systems, balance numbers, shape monetization, populate IAP catalog (full game, prototype, jam, live-game update). Use when user wants to design or tune a game and needs anything from a brief to a systems + balance + catalog plan. Triggers on `/game-shape`, "design a game", "game design doc", "balance the economy", "IAP catalog". Requires cold-context brief with goal, files_read, files_write, dependencies, conflicts, acceptance criteria, verification. For Godot implementation see godot-engineer; for engineering intake see prompt-shaping. For marketing or monetization strategy see game-marketer / game-monetization-strategist; for concept batches see game-concept-creator; for non-game marketing intake see marketing-shaper.
---
