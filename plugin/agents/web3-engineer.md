...
succeeds, record with `node "${CURSOR_PLUGIN_ROOT:-$HOME/.cursor/plugins/local/cursor-harness}/scripts/record-verify.mjs" --profile <node-harness|rust|custom> --run -- <cmd>`. Harness dogfood only: `npm run verify:record …`.
