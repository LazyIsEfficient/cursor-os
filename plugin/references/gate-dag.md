...
**Mechanical PR / push gate (local + CI):** after verification, record `.cursor/verify-ledger.json` (v2 + `--profile` + `--run` only) via `npm run verify:record -- --profile <node-harness|rust|custom> --run -- <cmd>` so `impl_verified` matches the current HEAD with profile coverage. **Consumer repos:** run `/verify-ci-parity` (or `npm run verify:ci-parity [--write]`) once (and again when CI workflows / justfile / Makefile change) to discover the CI check floor and optional `.cursor/verify-profile.json`; then `verify:record` those printed cmds. The `beforeShellExecution` guard denies plain `git push` (rule `git-push-without-verify`) and `gh pr create` / `gh pr ready` (rule `gh-pr-without-verify`) unless that ledger is valid; force / remote-ref-delete pushes stay hard-denied. A stop hook (`verify-ledger-stop`, `loop_limit: 1`) nudges once when the worktree is dirty or ahead without a valid ledger. CI `check-pr-ship-gates` requires a checked **impl-verified** checkbox on non-docs PRs. Emergency only: `VERIFY_PR_GATE_DISABLED=1` skips the shell-hook push+PR checks and the stop follow-up (not CI).
