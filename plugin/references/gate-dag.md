...
**Mechanical PR / push gate (local + CI):** after verification, record `.cursor/verify-ledger.json` (v2 + `--profile` + `--run` only) via `node "${CURSOR_PLUGIN_ROOT:-$HOME/.cursor/plugins/local/cursor-harness}/scripts/record-verify.mjs" --profile <node-harness|rust|custom> --run -- <cmd>` so `impl_verified` matches the current HEAD with profile coverage. **Consumer repos:** run `/verify-ci-parity` (or `node "…/scripts/ci-parity.mjs" [--write]`) once (and again when CI workflows / justfile / Makefile change) to discover the CI check floor and optional `.cursor/verify-profile.json`; then `record-verify` those printed cmds. Harness checkout only: `npm run verify:record` / `npm run verify:ci-parity` (not in consumer `package.json`). The `beforeShellExecution` guard denies plain `git push` (rule `git-push-without-verify`) and `gh pr create` / `gh pr ready` (rule `gh-pr-without-verify`) unless that ledger is valid; force / remote-ref-delete pushes stay hard-denied. A stop hook (`verify-ledger-stop`, `loop_limit: 1`) nudges once when the worktree is dirty or ahead without a valid ledger. CI `check-pr-ship-gates` requires a checked **impl-verified** checkbox on non-docs PRs. Emergency only: `VERIFY_PR_GATE_DISABLED=1` skips the shell-hook push+PR checks and the stop follow-up (not CI).

Rust ledger floor matches the skill CI shape above: `cargo clippy` must include `--all-targets` and `-D warnings` (or `-Dwarnings`) to satisfy the `rust` profile.
