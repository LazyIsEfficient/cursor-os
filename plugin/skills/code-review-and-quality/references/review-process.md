...
|--------|---------|---------------|
| **Critical:** | Highest-severity concern | Gates **only** when Tier 0 (an already-failing deterministic check) or Tier 1 with an attached reproducing artifact. Unevidenced, it is Tier 2 — advisory, logged, never a gate. |
| *(no prefix)* | Ordinary finding | Address it when it is Tier 0 or evidenced Tier 1; otherwise it is a proposal to the operator |
| **Nit:** | Minor, optional | Tier 2 — author may ignore. Formatting, style preferences |
...
Label any issues Critical / Nit / Optional / FYI, and state each
finding's tier. Critical gates only with Tier 0 or evidenced Tier 1
backing; otherwise it is Tier 2 and advisory.
```
...
- [ ] **Request changes** — at least one Tier 0 or evidenced Tier 1 finding is unresolved
- [ ] **Approve with advisories** — only Tier 2 findings remain; log them, do not gate on them
```
