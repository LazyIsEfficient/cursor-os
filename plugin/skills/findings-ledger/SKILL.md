...
Ledger stores reviewer judgment lacking deterministic evidence. Does not store Tier 0 failures or evidence-backed Tier 1 findings.
...
Normalize path and claim before deriving stable fingerprint. Repeated fingerprint becomes `RECURRING` only when it appears in at least two distinct `run_id` values; repetition within one review does not count.

## Triage

- `NEW`: advisory only.
- `RECURRING`: investigate whether deterministic check can encode claim.
