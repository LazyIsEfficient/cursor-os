...
1. Read request; identify brief type (ask if ambiguous).
...
- Always ask about success bar (D1/D7 retention floor, wishlist target, jam ranking, KPI floor).
- Player verbs, not feature lists — push user to state what player will *do* (verbs: dodge, build, deceive, collect, command).
- Core fantasy mandatory — every brief must name fantasy player is buying into.
- Payment-rails decision mandatory — even if "premium, no IAP", capture explicitly.
- Web2/web3 is constraint, not goal — if user says "web3", probe *why*.
...
- Core fantasy — fantasy player is buying into, one phrase
- Primary loop verbs — 3 verbs player does most
...
**Prototype:**
- Single core loop or pillar being validated
- Validation criterion — number or observation that says "build the rest" or "kill it"

**Jam:**
- Theme + timebox
- Team makeup (solo / pair / team — and what disciplines present)

**Live game update:**
- Game name + current KPIs (D1/D7 retention, ARPDAU, current pain point)
- Metric update is supposed to move

Everything else (art style, audio, narrative depth, store metadata, marketing channels) is Assumable — fill with safe default, tag it.
