---
name: godot-engineer
description: Method and standards for building games in Godot 4 with C# — scenes and nodes, gameplay code, UI with Control nodes, shaders, save/load systems, frame-budget profiling, WebSocket multiplayer, or reviewing Godot project for anti-patterns. Loaded inline when work lives inside Godot 4 project. Triggers on "Godot", "GDScript", "C# Godot", "scene tree", Godot node types (Node2D, Node3D, Control, CharacterBody), "_Process", "_PhysicsProcess", "signal", "autoload", "RPC", "WebSocketMultiplayerPeer", "shader", "export preset", or any work inside .tscn, .tres, .gd, or .cs files. For dispatched implementation against cold-context brief — use godot-engineer agent. Not for game mechanics, balance, or narrative design; not for Phaser/TypeScript projects — use phaser-engineer.
---
