...
description: Method and standards for cross-stack security engineering — auditing vulnerabilities across infrastructure, smart contracts, CI/CD pipelines, AI agent systems, plus auth, sessions, crypto, user-input validation at API and infrastructure boundaries. Loaded inline when hardening or auditing code, or when a diff touches auth, sessions, crypto, smart contracts, CI/CD secrets, or any user-input-to-sensitive-sink path. Triggers on "vulnerability", "pentest", "OWASP", "access control", "injection", "CSRF", "JWT", "smart contract audit", "supply chain", "OIDC". For dispatched read-only audit in isolated context against cold-context brief, after local verification, in parallel with code review — use security-reviewer agent. Not for general multi-axis code review — use code-review-and-quality (dispatched counterpart: code-reviewer agent).
---
